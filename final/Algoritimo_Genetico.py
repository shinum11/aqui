...
class ProblemaMochilaInterface:
    ...
    def executar_algoritmo_genetico(self):
        try:
            ...
            self.resultado_text.set("Erro: Insira valores válidos(Apenas valores númericos inteiros).")
            return
        except IndexError :
            self.resultado_text.set("Erro: O numero de pesos e valores tem que ser par (Min 4 entradas)")
            return
        ...
        historico_fitness = []
        melhor_solucao = None
        melhor_media_fitness = 0
        melhor_geracao = 0

        for i in range(geracoes):
            populacao = evolve(populacao, peso_maximo, pesos_e_valores, num_cromossomos, taxa_crossover, taxa_mutacao)
            media_fit = media_fitness(populacao, peso_maximo, pesos_e_valores)
            historico_cromossomos.append(populacao[0].copy())
            historico_fitness.append(media_fit)

            if media_fit > melhor_media_fitness:
                melhor_solucao = populacao[0].copy()
                melhor_media_fitness = media_fit
                melhor_geracao = i + 1

        # Atualiza a interface com o resultado
        resultado_final = f"Melhor Solução (Geração {melhor_geracao}):\n{melhor_solucao}\nValor Média Fitness:\n{melhor_media_fitness}"
